 ...
 */
@TeleOp(name="Evil Solo Teleop", group="8617")
public class ChanceEvilSoloTeleop extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addData("State", "freaky 😜");
        telemetry.update();
...
//          robot.waitForTick(40);
    ...
    void captureGamepad2Buttons() {
        gamepad2_triangle_last   = gamepad2_triangle_now;    gamepad2_triangle_now   = gamepad1.triangle;
        gamepad2_circle_last     = gamepad2_circle_now;      gamepad2_circle_now     = gamepad1.circle;
        gamepad2_cross_last      = gamepad2_cross_now;       gamepad2_cross_now      = gamepad1.cross;
        gamepad2_square_last     = gamepad2_square_now;      gamepad2_square_now     = gamepad1.square;
        gamepad2_dpad_up_last    = gamepad2_dpad_up_now;     gamepad2_dpad_up_now    = gamepad1.dpad_up;
        gamepad2_dpad_down_last  = gamepad2_dpad_down_now;   gamepad2_dpad_down_now  = gamepad1.dpad_down;
        gamepad2_dpad_left_last  = gamepad2_dpad_left_now;   gamepad2_dpad_left_now  = gamepad1.dpad_left;
        gamepad2_dpad_right_last = gamepad2_dpad_right_now;  gamepad2_dpad_right_now = gamepad1.dpad_right;
        gamepad2_l_bumper_last   = gamepad2_l_bumper_now;    gamepad2_l_bumper_now   = gamepad1.left_bumper;
        gamepad2_r_bumper_last   = gamepad2_r_bumper_now;    gamepad2_r_bumper_now   = gamepad1.right_bumper;
//      gamepad2_touchpad_last   = gamepad2_touchpad_now;    gamepad2_touchpad_now   = gamepad1.touchpad;
//      gamepad2_share_last      = gamepad2_share_now;       gamepad2_share_now      = gamepad1.share;
    } // captureGamepad2Buttons
...
//        return dPadMode
    ...
    void encoderReset (){
        if (gamepad2_triangle_last && !gamepad2_triangle_now) { // reset tele-op positions
    ...
    void processLift(){
        ...
        if (gamepad1.triangle) { // precaution for incorrect tele-op starting position
            gamepad1.runRumbleEffect(shortRumble);
            ...
            //precision movement
            if( (gamepad1.left_trigger >= 0.1) && (robot.slideRMotorPos >  0) ){
                if(gamepad1.left_trigger>=0.1 && gamepad1.left_trigger<= 0.3) {
                    robot.slideLMotor.setPower(-0.5*(gamepad1.left_trigger));
                    robot.slideRMotor.setPower(-0.5*(gamepad1.left_trigger));
                } else {
                    double motorPower = -1.3 * gamepad1.left_trigger;
                ...
                if (robot.chainMotorPos >= -940 && robot.slideRMotorPos > 1150) {
                    ...
                    robot.slideLMotor.setPower(.5*(gamepad1.right_trigger));
                    robot.slideRMotor.setPower(.5*(gamepad1.right_trigger));
                } else if (robot.chainMotorPos < -940) {
                    if(gamepad1.right_trigger>=0.1 && gamepad1.right_trigger<= 0.3) {
                        robot.slideLMotor.setPower(0.5*(gamepad1.right_trigger));
                        robot.slideRMotor.setPower(0.5*(gamepad1.right_trigger));
                    } else {
                        robot.slideLMotor.setPower(1 * (gamepad1.right_trigger));
                        robot.slideRMotor.setPower(1 * (gamepad1.right_trigger));
                    }
...
//        }
    ...
    void diffyMovement(){
        ...
        if(gamepad1.cross) {
            if (gamepad1.right_stick_y >= .25) { //rotate yaw up
                if (robot.diffyLPos > .015 && robot.diffyRPos < 0.985) {
                    robot.diffyLPos -= .015 * (gamepad1.right_stick_y);
                    robot.diffyRPos += .015 * (gamepad1.right_stick_y);
                } else {
                    gamepad1.runRumbleEffect(shortRumble);
                }
            } else if (gamepad1.right_stick_y <= -.25) { //rotate yaw down
            ...
            if (gamepad1.right_stick_x >= .25) { //rotate roll right
        ...
        if (robot.diffyRPos >= .90) { //single rumble when left is getting to high, one rumble when its going too low
            gamepad1.runRumbleEffect(rightDoubleRumble);
        } else if (robot.diffyRPos <= .10) {
            gamepad1.runRumbleEffect(rightRumble);
        }
        //set pos
    ...
    void clawMovement() {
        //toggle for claw
        if (gamepad1_circle_now && !gamepad1_circle_last) { //controller input
